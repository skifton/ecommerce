...
const EFooter: React.FC = () => {
  const { categories: footerShopCategories } = useGetCategoryList();
  return (
    <footer aria-labelledby="footer-heading" className="bg-gray-900 mt-40">
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
            ...
            <div className="space-y-12 md:grid md:grid-cols-2 md:gap-8 md:space-y-0">
              <div>
                ...
                <ul className="mt-6 space-y-6">
                  {footerShopCategories?.map((item) => (
                    <li key={item.name} className="text-sm">
                      <Link
                        to={{ pathname: '/category', search: `?name=${item.name}` }}
        ...
        <div className="border-t border-gray-800 py-10">
          <p className="text-sm text-gray-400">
            Copyright &copy; 2023 Anton Simanenkau, Inc.
          </p>
        </div>
      </div>
    </footer>
